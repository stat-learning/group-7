...
While we initially wanted to split our data into training and test datasets to simulate the test MSE of our model, this proved impossible. Since there are levels in some of the categorical variables that have very few observations, it was not possible to get a test dataset which did not have levels that the training dataset had not seen before, resulting in a N/A value for the test MSE. As a next-best, we calculated the training MSE of our models for comparison.
...
To get an estimate of the accuracy of this model, we estimate our training mean squared error. 

```{r Test MSE for linear model without imputation}
MSE_lin = (df_final_section$log_md_wage - predict.lm(linear.model.1, df_final_section)) ^ 2
MSE_lin
```

```{r Linear model with mice imputation}
...
```{r Linear model with missForest imputation}
...
Here, the lasso regression resulted in 

```{r Lasso model with mice imputation}
# Data setup
lasso.data <- cbind(numerics.section,
                    categoricals[1:2634,])
lasso.data <- lasso.data %>%
...
lasso.pred = predict(lasso.1, s = bestlam1, X)
