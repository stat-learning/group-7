...
df_final <- cbind(numerics, 
                  REGION = section1$REGION, 
                  CONTROL = section1$CONTROL, 
                  HIGHDEG = section1$HIGHDEG)

# Convert categoricals to factors
df_final$REGION <- factor(df_final$REGION) 
df_final$CONTROL <- factor(df_final$CONTROL)
df_final$HIGHDEG <- factor(df_final$HIGHDEG)
```
...
```{r}
library(missForest)
df_imp <- missForest(df_final)
```

```{r}
imputed_df <- df_imp$ximp
```
